    ...
    <IntlProvider locale="en" messages={{}}>
      <QueryClientProvider client={queryClient}>
        <MemoryRouter initialEntries={['/home']}>
          <Routes>
            <Route path="/home" element={<StudioHome intl={injectIntl} />} />
            <Route
              path="/libraries"
...
describe('<StudioHome />', () => {
  describe('api fetch fails', () => {
    beforeEach(async () => {
      ...
      useSelector.mockReturnValue({
        studioHomeLoadingStatus: RequestStatus.FAILED
      })
    })
    ...
    it('should render fetch error', () => {
      ...
      expect(
        getByText(messages.homePageLoadFailedMessage.defaultMessage)
      ).toBeInTheDocument()
    })
  ...
  describe('api fetch succeeds', () => {
    ...
    it('should render email staff header button', async () => {
      ...
      expect(
        getByRole('link', { name: messages.emailStaffBtnText.defaultMessage })
    ...
    it('should render create new course button', async () => {
      ...
      expect(
        getByRole('button', {
          name: messages.addNewCourseBtnText.defaultMessage
        })
      ).toBeInTheDocument()
    })
    ...
    it('should show verify email layout if user inactive', () => {
      ...
      const { getByText } = render(<RootWrapper />)
      expect(
        getByText('Thanks for signing up, abc123!', { exact: false })
      ).toBeInTheDocument()
    })
    ...
    it('should render create new course container', async () => {
      ...
      const createNewCourseButton = getByRole('button', {
        name: messages.addNewCourseBtnText.defaultMessage
      })

      await act(() => fireEvent.click(createNewCourseButton))
      expect(
        getByText(createNewCourseMessages.createNewCourse.defaultMessage)
      ).toBeInTheDocument()
    })
    ...
    it('should hide create new course container', async () => {
      ...
      const cancelButton = getByRole('button', {
        name: createOrRerunCourseMessages.cancelButton.defaultMessage
      })
      fireEvent.click(cancelButton)
      waitFor(() => {
        expect(
          queryByText(createNewCourseMessages.createNewCourse.defaultMessage)
    ...
    describe('contact administrator card', () => {
      it('should show contact administrator card with no add course buttons', () => {
        ...
        const defaultTitleMessage =
          messages.defaultSection_1_Title.defaultMessage
        const titleWithStudioName = defaultTitleMessage.replace(
          '{studioShortName}',
          'Studio'
        )
        ...
        const addCourseButton = queryByText(
